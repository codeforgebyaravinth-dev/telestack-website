...
Read this document carefully before making PRs to the TeleStack Website repo.

## What goes in docs?

The TeleStack documentation is meant to provide general guidance that's:

- Unopinionated
- Focused on the correct use of TeleStack services (Appwrite, LiveKit, Matrix)
...
- Examples that only include a select subset of TeleStack SDKs.
- Examples that do not work for all developers using TeleStack, but specific to TeleStack + technology.

Note that the tutorials and blogs available on the TeleStack blog and docs are meant for these types of information.

## Documentation structure

The TeleStack docs is split into sections, each with its intended purpose and content.

TeleStack's navigation increases in complexity from top down. We expect users to view links later in navigation later in their development journey.
